...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        await enviaEmail(pedido.adotante.nome, pedido.adotante.email, descricao, "Recebemos seu pedido de adoção. Em breve você receberá a análise do nosso time.");
...
router.patch("/:id", verificaToken_1.verificaToken, async (req, res) => {
    ...
    if (!resposta)
        return res.status(400).json({ erro: "Informe a resposta deste pedido." });
    try {
